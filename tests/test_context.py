...
class TestBuildContextProfile:
    def test_goal_from_profile(self):
        profile = MockProfile(
            max_hr=190,
            active_goal_key="run_10k_60",
        )
        ctx = _build_context([], [], profile)
        assert ctx.goal_label == "10 км за 60 минут"
        assert ctx.goal_distance_km == 10.0
        assert ctx.goal_target_time_min == 60

    def test_training_days_from_profile(self):
        import json
        profile = MockProfile(
            available_training_days=json.dumps(["mon", "tue", "thu", "sat"]),
            max_run_days_per_week=3,
            strength_days_per_week=1,
        )
        ctx = _build_context([], [], profile)
        assert ctx.available_training_days == ["Пн", "Вт", "Чт", "Сб"]
        assert ctx.max_run_days_per_week == 3
        assert ctx.strength_days_per_week == 1

    def test_day_of_week_always_set(self):
        ctx = _build_context([], [], None)
        assert ctx.day_of_week is not None  # always set regardless of profile

    def test_no_goal_without_profile(self):
        ctx = _build_context([], [], None)
        assert ctx.goal_label is None

    def test_unknown_goal_key(self):
        profile = MockProfile(active_goal_key="nonexistent_goal")
        ctx = _build_context([], [], profile)
        assert ctx.goal_label is None
